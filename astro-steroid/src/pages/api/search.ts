...
// Helper to clean slug - removes /steroids/ prefix and leading slashes
const cleanSlug = (slug: string) => slug.replace(/^\/steroids\//, '').replace(/^\//, '');
...
const staticPages = [
  ...
  { title: 'Shop All Products', slug: '/shop/', keywords: ['shop', 'all', 'products', 'browse', 'catalog'] },
];
  ...
  try {
    ...
    const productResults = products
      ...
      .map(product => ({
        ...
        title: product.data.title.replace(' | Steroiden UK', '').replace('Buy ', ''),
        slug: `/steroids/${cleanSlug(product.slug)}`,
        category: product.data.category?.replace(/-/g, ' ') || 'Product',
