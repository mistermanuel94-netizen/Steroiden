...
function saveAuthToStorage() {
  if (isBrowser) {
    ...
    if (user) {
      localStorage.setItem('steroid-user', JSON.stringify(user));
      localStorage.setItem('steroid-orders', JSON.stringify(userOrders.get()));
      localStorage.setItem('steroid-addresses', JSON.stringify(userAddresses.get()));
    } else {
      localStorage.removeItem('steroid-user');
      localStorage.removeItem('steroid-orders');
      localStorage.removeItem('steroid-addresses');
    }
  }
}
...
function checkGoogleAuthCookie() {
  ...
  const authCookie = getCookie('steroid-google-auth');
  ...
  if (authCookie) {
    ...
    try {
      ...
      deleteCookie('steroid-google-auth');
    } catch (e) {
...
if (isBrowser) {
  ...
  if (!currentUser.get()) {
    const savedUser = localStorage.getItem('steroid-user');
    const savedOrders = localStorage.getItem('steroid-orders');
    const savedAddresses = localStorage.getItem('steroid-addresses');
...
export async function registerWithEmail(
  ...
  const existingUsers = JSON.parse(localStorage.getItem('steroid-all-users') || '[]');
  ...
  localStorage.setItem('steroid-all-users', JSON.stringify(existingUsers));
...
export async function loginWithEmail(email: string, password: string): Promise<User> {
  ...
  // Load user's orders
  const allOrders = JSON.parse(localStorage.getItem('steroid-all-orders') || '[]');
...
export function addOrder(order: Omit<Order, 'id' | 'date'>): Order {
  ...
  if (user) {
    ...
    localStorage.setItem('steroid-all-orders', JSON.stringify(allOrders));
