...
export default function Dashboard({ lang = 'en' }: DashboardProps) {
  ...
  const getStatusColor = (status: Order['status']) => {
    switch (status) {
      ...
      case 'processing': return { bg: '#E0D8D0', text: '#3730a3' };
      case 'shipped': return { bg: '#d1fae5', text: '#065f46' };
      case 'delivered': return { bg: '#dcfce7', text: '#166534' };
      default: return { bg: '#EDE8E3', text: '#555555' };
    }
  };
    ...
    <div className="container dashboard-container" style={{ paddingTop: '40px', paddingBottom: '60px' }}>
      <div className="row">
        ...
        <div className="col-lg-3 mb-4">
          <div style={{ ...cardStyle, padding: '24px', marginBottom: '20px' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: '24px' }}>
              ...
              <h5 style={{ color: '#1A1A1A', fontWeight: '700', marginBottom: '4px' }}>
                {$currentUser?.firstName} {$currentUser?.lastName}
              </h5>
              <p style={{ color: '#6B6B6B', fontSize: '14px', marginBottom: '0' }}>{$currentUser?.email}</p>
              {$currentUser?.provider && $currentUser.provider !== 'email' && (
                <span style={{
                  ...
                  gap: '4px',
                  marginTop: '8px',
                  background: $currentUser.provider === 'google' ? '#EDE8E3' : '#eff6ff',
                  color: $currentUser.provider === 'google' ? '#3D3D3D' : '#1d4ed8',
                  padding: '4px 10px',
                  borderRadius: '20px',
                  fontSize: '12px',
                  fontWeight: '500'
        ...
        <div className="col-lg-9">
            ...
            <>
              ...
              <div className="row">
                <div className="col-md-4 mb-4">
                  <div style={{ ...cardStyle, padding: '24px' }}>
                    ...
                    <h2 style={{ color: '#1A1A1A', fontWeight: '800', marginBottom: '4px' }}>{$userOrders.length}</h2>
                    <p style={{ color: '#6B6B6B', fontSize: '14px', marginBottom: '0' }}>{t(lang, 'accountPage.totalOrders')}</p>
                  </div>
                </div>
                <div className="col-md-4 mb-4">
                  <div style={{ ...cardStyle, padding: '24px' }}>
                    ...
                    <h2 style={{ color: '#1A1A1A', fontWeight: '800', marginBottom: '4px' }}>
                      {$userOrders.filter(o => o.status === 'delivered').length}
                    </h2>
                    <p style={{ color: '#6B6B6B', fontSize: '14px', marginBottom: '0' }}>{t(lang, 'accountPage.completed')}</p>
                  </div>
                </div>
                <div className="col-md-4 mb-4">
                  <div style={{ ...cardStyle, padding: '24px' }}>
                    ...
                    <p style={{ color: '#6B6B6B', fontSize: '14px', marginBottom: '0' }}>{t(lang, 'accountPage.inProgress')}</p>
                  </div>
                </div>
              </div>
              ...
              <div style={{ ...cardStyle, padding: '24px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                  <h5 style={{ color: '#1A1A1A', fontWeight: '700', marginBottom: '0' }}>{t(lang, 'accountPage.recentOrders')}</h5>
                  <button
                ...
                {$userOrders.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '40px 20px' }}>
                    <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#C4BDB5" strokeWidth="1.5" style={{ marginBottom: '16px' }}>
                      <path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4Z"></path>
                      <path d="M3 6h18"></path>
                      <path d="M16 10a4 4 0 0 1-8 0"></path>
                    </svg>
                    <p style={{ color: '#6B6B6B', marginBottom: '16px' }}>{t(lang, 'accountPage.noOrders')}</p>
                    <a
                      href="/shop/"
                      style={{
                        display: 'inline-flex',
                    ...
                    {$userOrders.slice(0, 3).map(order => {
                      ...
                      return (
                        <div key={order.id} style={{ 
                          ...
                          <div>
                            <div style={{ fontWeight: '600', color: '#1A1A1A', marginBottom: '4px' }}>{order.id}</div>
                            <div style={{ color: '#6B6B6B', fontSize: '13px' }}>{formatDate(order.date)}</div>
                          </div>
          ...
          {activeTab === 'orders' && (
            <div style={{ ...cardStyle, padding: '24px' }}>
              <h4 style={{ color: '#1A1A1A', fontWeight: '700', marginBottom: '24px' }}>{t(lang, 'accountPage.myOrders')}</h4>
              {$userOrders.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '60px 20px' }}>
                  <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#C4BDB5" strokeWidth="1.5" style={{ marginBottom: '20px' }}>
                    <path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4Z"></path>
                    <path d="M3 6h18"></path>
                    <path d="M16 10a4 4 0 0 1-8 0"></path>
                  </svg>
                  <h5 style={{ color: '#1A1A1A', marginBottom: '8px' }}>{t(lang, 'accountPage.noOrders')}</h5>
                  <p style={{ color: '#6B6B6B', marginBottom: '24px' }}>{t(lang, 'accountPage.noOrdersDesc')}</p>
                  <a
                    href="/shop/"
                    style={{
                      display: 'inline-flex',
                    ...
                    return (
                      <div key={order.id} style={{ 
                        ...
                        <div style={{ 
                          ...
                          <div>
                            <div style={{ fontWeight: '700', color: '#1A1A1A', marginBottom: '4px', fontSize: '1.1rem' }}>{order.id}</div>
                            <div style={{ color: '#6B6B6B', fontSize: '14px' }}>{t(lang, 'accountPage.placedOn')} {formatDate(order.date)}</div>
                          </div>
                        ...
                        <div style={{ padding: '20px' }}>
                          {order.items.map((item, idx) => (
                            <div key={idx} style={{ 
                              ...
                              justifyContent: 'space-between',
                              padding: '8px 0',
                              borderBottom: idx < order.items.length - 1 ? '1px solid #EDE8E3' : 'none'
                            }}>
                              <span style={{ color: '#1A1A1A' }}>{item.title} × {item.quantity}</span>
          ...
          {activeTab === 'addresses' && (
            <div style={{ ...cardStyle, padding: '24px' }}>
              <h4 style={{ color: '#1A1A1A', fontWeight: '700', marginBottom: '24px' }}>{t(lang, 'accountPage.savedAddresses')}</h4>
              {$userAddresses.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '60px 20px' }}>
                  ...
                  <h5 style={{ color: '#1A1A1A', marginBottom: '8px' }}>{t(lang, 'accountPage.noAddresses')}</h5>
                  <p style={{ color: '#6B6B6B', marginBottom: '0' }}>{t(lang, 'accountPage.addressesSavedDuringCheckout')}</p>
                </div>
              ) : (
                <div className="row">
                  {$userAddresses.map(address => (
                    <div key={address.id} className="col-md-6 mb-3">
                      <div style={{ 
                        padding: '20px',
                        border: address.isDefault ? '2px solid var(--accent-primary)' : '1px solid #E0D8D0',
                        borderRadius: '12px',
          ...
          {activeTab === 'settings' && (
            <div style={{ ...cardStyle, padding: '32px' }}>
              <h4 style={{ color: '#1A1A1A', fontWeight: '700', marginBottom: '28px' }}>{t(lang, 'accountPage.accountSettings')}</h4>
              
              <div style={{ marginBottom: '32px' }}>
                <h5 style={{ color: '#1A1A1A', fontWeight: '600', marginBottom: '20px' }}>{t(lang, 'accountPage.profileInfo')}</h5>
                {isEditing ? (
                  <div>
                    <div className="row">
                      <div className="col-md-6 mb-3">
                        <label style={{ display: 'block', color: '#6B6B6B', fontWeight: '600', marginBottom: '8px', fontSize: '14px' }}>{t(lang, 'accountPage.firstName')}</label>
                        <input 
                          type="text"
                      ...
                      <div className="col-md-6 mb-3">
                        <label style={{ display: 'block', color: '#6B6B6B', fontWeight: '600', marginBottom: '8px', fontSize: '14px' }}>{t(lang, 'accountPage.lastName')}</label>
                        <input 
                          type="text"
                    ...
                    <div className="mb-3">
                      <label style={{ display: 'block', color: '#6B6B6B', fontWeight: '600', marginBottom: '8px', fontSize: '14px' }}>{t(lang, 'accountPage.phoneOptional')}</label>
                      <input 
                        type="tel"
                  ...
                  <div>
                    <div style={{ display: 'grid', gap: '16px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 16px', background: '#F5F0EB', borderRadius: '10px' }}>
                        <span style={{ color: '#6B6B6B' }}>{t(lang, 'common.name')}</span>
                        <span style={{ fontWeight: '600', color: '#1A1A1A' }}>{$currentUser?.firstName} {$currentUser?.lastName}</span>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 16px', background: '#F5F0EB', borderRadius: '10px' }}>
                        <span style={{ color: '#6B6B6B' }}>{t(lang, 'common.email')}</span>
                        <span style={{ fontWeight: '600', color: '#1A1A1A' }}>{$currentUser?.email}</span>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 16px', background: '#F5F0EB', borderRadius: '10px' }}>
                        <span style={{ color: '#6B6B6B' }}>{t(lang, 'common.phone')}</span>
                        <span style={{ fontWeight: '600', color: '#1A1A1A' }}>{$currentUser?.phone || t(lang, 'common.notSet')}</span>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 16px', background: '#F5F0EB', borderRadius: '10px' }}>
                        <span style={{ color: '#6B6B6B' }}>{t(lang, 'accountPage.memberSince')}</span>
                        <span style={{ fontWeight: '600', color: '#1A1A1A' }}>{$currentUser?.createdAt ? formatDate($currentUser.createdAt) : 'N/A'}</span>
                      </div>
                    </div>
                    <button
