...
// Pack options: multiples of 6 starting from nearest multiple >= MOQ
const MULTIPLES_OF_6 = [6, 12, 18, 24, 30, 36];
const generatePackOptions = (moq: number): number[] => {
  const startIdx = MULTIPLES_OF_6.findIndex(m => m >= Math.max(1, moq));
  // Return from the first qualifying multiple through end of list
  return startIdx >= 0 ? MULTIPLES_OF_6.slice(startIdx) : [MULTIPLES_OF_6[MULTIPLES_OF_6.length - 1]];
};
...
export default function ProductOptions({ id, title, basePrice, packageSizes, moq, image, lang }: ProductOptionsProps) {
  // Generate pack options starting from MOQ
  const PACK_OPTIONS = generatePackOptions(moq);
  // Pack quantity starts at first multiple of 6 >= MOQ
  const [packQuantity, setPackQuantity] = useState(PACK_OPTIONS[0]);
    ...
    <div className="product-options">
      ...
      <div className="price-display mb-4 p-4 rounded" style={{ background: 'linear-gradient(135deg, #f0f9ff 0%, #e0f2fe 100%)', border: '1px solid #bae6fd' }}>
        ...
        <div className="d-flex justify-content-between align-items-center">
          ...
          {pricePerUnit && (
            <span className="small" style={{ color: '#6B6B6B' }}>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  minWidth: '100px',
                  padding: '12px 4px',
                  border: packQuantity === qty ? '2px solid var(--accent-primary)' : '2px solid #E0D8D0',
                  borderRadius: '10px',
                  background: packQuantity === qty ? 'rgba(26, 46, 90, 0.10)' : 'white',
                  cursor: 'pointer',
              ...
              >
                <div className="pack-qty font-weight-bold" style={{ fontSize: '1rem', color: '#1A1A1A' }}>
                  {qty}
                  <span style={{ 
                    display: 'block', 
                    fontWeight: 'normal', 
                ...
                <div className="pack-price" style={{ color: packMeetsThreshold ? 'var(--accent-primary)' : '#6B6B6B', fontWeight: '600' }}>
      ...
      {/* Order Summary */}
      <div className="order-summary mb-4 p-4 rounded" style={{ background: '#F5F0EB', border: '1px solid #E0D8D0' }}>
        <h6 className="font-weight-bold mb-3" style={{ color: '#1A1A1A' }}>{t(lang, 'cart.orderSummary')}</h6>
        ...
        {/* Total */}
        <div className="d-flex justify-content-between align-items-center pt-3 mt-2" style={{ borderTop: '2px solid #E0D8D0' }}>
          <span className="font-weight-bold" style={{ fontSize: '1.1rem', color: '#1A1A1A' }}>{t(lang, 'cart.total')}</span>
          <span className="font-weight-bold text-primary" style={{ fontSize: '1.5rem' }}>
        ...
        {meetsThreshold && discountAmount > 0 && (
          <div className="text-center mt-3">
            ...
            {qualifiesForFreeDelivery && (
              <span className="badge ml-2" style={{ background: '#1A2E5A', color: 'white', padding: '8px 16px', fontSize: '0.85rem' }}>
                + {t(lang, 'cart.freeDelivery')}
              </span>
            )}
          </div>
      ...
      {/* Trust Badges */}
      <div className="trust-badges mt-4 p-3 rounded" style={{ background: '#F5F0EB' }}>
        <div className="d-flex flex-wrap justify-content-center" style={{ gap: '12px' }}>
          <span className="d-flex align-items-center small" style={{ color: '#6B6B6B' }}>
      ...
      <style>{`
        ...
        .pack-btn.active {
          box-shadow: 0 0 0 3px rgba(26, 46, 90, 0.20);
        }
      ...
      >
        <div className="d-flex align-items-center justify-content-between gap-3">
          <div className="d-flex flex-column">
            <span style={{ fontSize: '0.75rem', color: '#6B6B6B' }}>{t(lang, 'cart.total')}</span>
          ...
          <div className="d-flex align-items-center gap-2">
            <div className="d-flex align-items-center border rounded-3" style={{ height: '48px', border: '2px solid #E0D8D0' }}>
               <button 
                className="btn btn-link text-dark p-0" 
                style={{ width: '44px', height: '44px', textDecoration: 'none', fontSize: '20px' }}
                onClick={() => { const idx = PACK_OPTIONS.indexOf(packQuantity); if (idx > 0) setPackQuantity(PACK_OPTIONS[idx - 1]); }}
              >−</button>
              <span className="font-weight-bold px-2" style={{ minWidth: '32px', textAlign: 'center' }}>{packQuantity}</span>
              <button 
                className="btn btn-link text-dark p-0"
                style={{ width: '44px', height: '44px', textDecoration: 'none', fontSize: '20px' }}
                onClick={() => { const idx = PACK_OPTIONS.indexOf(packQuantity); if (idx < PACK_OPTIONS.length - 1) setPackQuantity(PACK_OPTIONS[idx + 1]); }}
              >+</button>
            </div>
