  ...
  return (
    <>
      ...
      <style>{`
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {isEmpty ? (
            <div className="text-center py-10 text-slate-500">
              <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#C4BDB5" strokeWidth="1.5" className="mx-auto mb-4" style={{display:'block', margin:'0 auto 16px'}}>
              ...
              <a 
                href={getLocalizedPath('/steroids/', lang)}
                onClick={handleClose}
            ...
            <div className="flex flex-col gap-4" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {items.map((item) => (
                <div 
                  ...
                  style={{ display: 'flex', gap: '12px', padding: '12px', background: '#F5F0EB', borderRadius: '10px' }}
                >
                  <img 
                    src={item.thumb_src} 
                    alt={item.title}
                    ...
                    onError={(e) => {
                      (e.target as HTMLImageElement).src = '/images/steroid-default.jpg';
                    }}
                  />
                  <div className="flex-1 min-w-0 flex flex-col justify-between py-1" style={{ flex: 1 }}>
                    ...
                    <div className="flex justify-between items-end mt-2" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: '8px' }}>
                      ...
                      <div className="flex items-center bg-white border border-slate-200 rounded-lg h-8" style={{ display: 'flex', alignItems: 'center', border: '1px solid #E0D8D0', borderRadius: '6px' }}>
                        <button
        ...
        {/* Footer */}
        {!isEmpty && (
          <div className="p-4 border-t border-gray-100 bg-white pb-[max(1rem,env(safe-area-inset-bottom))]" style={{ padding: '16px', borderTop: '1px solid #EDE8E3', paddingBottom: 'max(16px, env(safe-area-inset-bottom))' }}>
             {/* Subtotal */}
            <div className="flex justify-between items-end mb-1" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
             ...
             {/* Shipping Note */}
            <div className="text-xs text-slate-400 mb-4 text-right" style={{ fontSize: '0.75rem', color: '#A0A0A0', textAlign: 'right', marginBottom: '16px' }}>
            ...
            <div className="flex flex-col gap-3" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                ...
                style={{ width: '100%', background: 'none', border: 'none', padding: '8px', color: '#6B6B6B', cursor: 'pointer' }}
                >
                  Continue Shopping
                </button>
            </div>
