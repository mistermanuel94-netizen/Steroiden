...
export default function ProductCard({ id, title, price, image, slug, category, rating = 4.5, reviewCount = 121, packageSize, lang = 'en', labels }: ProductCardProps) {
  ...
  const productUrl = getLocalizedPath(`/steroids/${slug.replace(/^\//, '')}`, lang);
  ...
  const renderStars = () => {
    ...
    for (let i = 0; i < 5; i++) {
      if (i < fullStars) {
        stars.push(
          <svg key={i} width="14" height="14" viewBox="0 0 24 24" fill="#C9A96E" stroke="#C9A96E" strokeWidth="1">
            <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
          </svg>
        );
      } else if (i === fullStars && hasHalfStar) {
        stars.push(
          <svg key={i} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#C9A96E" strokeWidth="1">
            <defs>
        ...
        stars.push(
          <svg key={i} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#C4BDB5" strokeWidth="1">
            <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
          </svg>
        );
      }
  ...
  return (
    <div className="shopcart-product-card">
      ...
      >
        <svg width="20" height="20" viewBox="0 0 24 24" fill={wishlisted ? "#ef4444" : "none"} stroke={wishlisted ? "#ef4444" : "#6B6B6B"} strokeWidth="2">
